...
class TimelineConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="timeline_lattice", port=50051
    ))
...
class HiveConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="hive_orchestrator", port=50052
    ))
    max_agents: int = 100
...
class MemoryConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="memory_inversion", port=50053
    ))
    max_snapshots: int = 1000
    snapshot_interval_seconds: int = 60
    inversion_strategies: List[str] = field(default_factory=lambda: [
        "contradiction", "temporal_shift", "identity_swap", "semantic_inversion"
    ])
...
class LoopsConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="recursive_loop_synth", port=50054
    ))
...
class OmegaConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="omega_register", port=50055
    ))
...
class TelemetryConfig:
    ...
    service: ServiceConfig = field(default_factory=lambda: ServiceConfig(
        name="telemetry_exporter", port=50056
    ))
...
class FrameworkConfig:
    ...
    def from_env(cls) -> 'FrameworkConfig':
        ...
        config.database.keyspace = os.getenv("SCYLLA_KEYSPACE", config.database.keyspace)
        config.database.username = os.getenv("SCYLLA_USERNAME")
        config.database.password = os.getenv("SCYLLA_PASSWORD")
        config.database.ssl_enabled = os.getenv("SCYLLA_TLS_ENABLED", "false").lower() == "true"
        config.database.auth_enabled = os.getenv("SCYLLA_AUTH_ENABLED", "false").lower() == "true"
        ...
        config.redis.ssl_enabled = os.getenv("REDIS_TLS_ENABLED", "false").lower() == "true"

        # Ray configuration
        config.ray.head_node_host = os.getenv("RAY_HEAD_NODE_HOST", config.ray.head_node_host)
        config.ray.head_node_port = int(os.getenv("RAY_HEAD_NODE_PORT", config.ray.head_node_port))
        ...
        config.monitoring.prometheus_enabled = os.getenv("PROMETHEUS_ENABLED", "true").lower() == "true"
        ...
        # Service configurations
        config.timeline.service.host = os.getenv("TIMELINE_HOST", config.timeline.service.host)
        config.timeline.service.port = int(os.getenv("TIMELINE_PORT", config.timeline.service.port))
        config.timeline.max_timelines = int(os.getenv("TIMELINE_MAX_TIMELINES", config.timeline.max_timelines))
        ...
        config.hive.max_agents = int(os.getenv("HIVE_MAX_AGENTS", config.hive.max_agents))

        config.memory.service.host = os.getenv("MEMORY_HOST", config.memory.service.host)
        config.memory.service.port = int(os.getenv("MEMORY_PORT", config.memory.service.port))

        config.loops.service.host = os.getenv("LOOPS_HOST", config.loops.service.host)
        config.loops.service.port = int(os.getenv("LOOPS_PORT", config.loops.service.port))

        config.omega.service.host = os.getenv("OMEGA_HOST", config.omega.service.host)
        config.omega.service.port = int(os.getenv("OMEGA_PORT", config.omega.service.port))

        config.telemetry.service.host = os.getenv("TELEMETRY_HOST", config.telemetry.service.host)
        config.telemetry.service.port = int(os.getenv("TELEMETRY_PORT", config.telemetry.service.port))
