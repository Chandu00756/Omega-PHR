...
class HiveConfig:
    ...
    max_agents: int = field(
        default_factory=lambda: int(os.getenv("HIVE_MAX_AGENTS", "100"))
    )
    agent_spawn_rate: float = field(
        default_factory=lambda: float(os.getenv("HIVE_SPAWN_RATE", "2.0"))
    )
    agent_diversity_factor: float = field(
        default_factory=lambda: float(os.getenv("HIVE_DIVERSITY", "0.8"))
    )

    # Attack Strategy Configuration
    attack_intensity: float = field(
        default_factory=lambda: float(os.getenv("HIVE_INTENSITY", "0.7"))
    )
    coordination_protocol: str = field(
        default_factory=lambda: os.getenv("HIVE_PROTOCOL", "swarm")
    )
    adaptation_rate: float = field(
        default_factory=lambda: float(os.getenv("HIVE_ADAPTATION", "0.3"))
    )

    # Swarm Intelligence Parameters
    pheromone_decay: float = field(
        default_factory=lambda: float(os.getenv("HIVE_PHEROMONE_DECAY", "0.1"))
    )
    exploration_factor: float = field(
        default_factory=lambda: float(os.getenv("HIVE_EXPLORATION", "0.4"))
    )
    collective_memory_size: int = field(
        default_factory=lambda: int(os.getenv("HIVE_MEMORY_SIZE", "1000"))
    )
    ...
    max_attack_duration: int = field(
        default_factory=lambda: int(os.getenv("HIVE_MAX_DURATION", "3600"))
    )
    emergency_shutdown_threshold: float = field(
        default_factory=lambda: float(os.getenv("HIVE_SHUTDOWN_THRESHOLD", "0.95"))
    )
    attack_success_limit: int = field(
        default_factory=lambda: int(os.getenv("HIVE_SUCCESS_LIMIT", "50"))
    )
    ...
    # Data Storage
    attack_logs_path: str = field(
        default_factory=lambda: os.getenv("HIVE_LOGS_PATH", "./data/hive_logs")
    )
    strategy_bank_path: str = field(
        default_factory=lambda: os.getenv("HIVE_STRATEGY_BANK", "./data/strategies")
    )
    checkpoint_interval: int = field(
        default_factory=lambda: int(os.getenv("HIVE_CHECKPOINT_INTERVAL", "300"))
    )
    ...
    ray_head_port: int = field(
        default_factory=lambda: int(os.getenv("RAY_HEAD_PORT", "10001"))
    )
    ray_memory_limit: int = field(
        default_factory=lambda: int(os.getenv("RAY_MEMORY_LIMIT", "4096"))
    )
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            assert (
                0.0 <= self.attack_intensity <= 1.0
            ), "attack_intensity must be in [0,1]"
            assert (
                0.0 <= self.adaptation_rate <= 1.0
            ), "adaptation_rate must be in [0,1]"
            assert (
                0.0 <= self.pheromone_decay <= 1.0
            ), "pheromone_decay must be in [0,1]"
            assert (
                0.0 <= self.exploration_factor <= 1.0
            ), "exploration_factor must be in [0,1]"
            assert (
                self.collective_memory_size > 0
            ), "collective_memory_size must be positive"
            assert self.max_attack_duration > 0, "max_attack_duration must be positive"
            assert (
                0.0 <= self.emergency_shutdown_threshold <= 1.0
            ), "shutdown_threshold must be in [0,1]"
            return True
